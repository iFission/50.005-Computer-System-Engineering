...
void job_dispatch(int i)
{
    ...
    while (true)
    {
        ...
        if (shmPTR_jobs_buffer->task_status != -1)
        {
            switch (shmPTR_jobs_buffer->task_type)
            {
            case 't':
                task(shmPTR_jobs_buffer->task_duration);
            case 'w':
                usleep(shmPTR_jobs_buffer->task_duration * TIME_MULTIPLIER);
            case 'z':
                exit(3);
            case 'i':
                kill(getpid(), SIGKILL);
