...
public class DesImageSolutionBottomUp {
    public static void main(String[] args) throws Exception {
        ...
        for (int idx = 0; idx < image_width; idx++) {
            ...
            System.out.println(each_width_pixel.length);
            System.out.println(encrypted_each_width_pixel.length);
            ...
            IntBuffer intBuf = ByteBuffer.wrap(encrypted_each_width_pixel).order(ByteOrder.BIG_ENDIAN).asIntBuffer();
            int[] array = new int[intBuf.remaining()];
            intBuf.get(array);
            System.out.println(array.length);

            for (int idy = 0; idy < image_length; idy++) {
                outImage.setRGB(idx, idy, array[image_length - idy - 1]);

            }

            // 415 horizontal pixels, 247 vertical
            // loops through each of the 415 (x)
            // each vertical line encrypt
        }
        //write outImage into file
        ImageIO.write(outImage, "BMP", new File("EnSUTD_CBC_BottomUp.bmp"));
    }
}
